...
export function VideoInfoCard({ video, themeName }: { video: VideoInfo; themeName?: string }) {
  return (
    <div className="flex animate-fade-up flex-col gap-4 rounded-2xl border border-line bg-surface p-4 sm:p-5">
      <div className="flex items-center gap-2 px-1">
        <span className="h-1.5 w-1.5 rounded-full bg-amber" />
        <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-ink-muted">
          Detected on {themeName || video.source}
        </p>
      </div>
